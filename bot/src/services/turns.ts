 ...
 * I/O Telegram aqui (status, diff por arquivo, balão de resposta, think, typing).
 *
 * Um turno = um balão de status (atividade atual + recentes, editado no
 * lugar) + uma mensagem por arquivo alterado (diff) + resposta final.
 * Sem balão por ferramenta: o passado recente vive no status.
 */
...
export interface LiveTurn {
  ...
  resultLast: string;
  /** path -> fingerprint do diff já enviado (evita reenviar acumulado). */
  diffSent: Map<string, string>;
  typingTimer: ReturnType<typeof setInterval> | null;
  streamTimer: ReturnType<typeof setInterval> | null;
  /** Eventos SSE dobrados (diagnóstico: finish sem texto + 0 eventos = nada chegou). */
  events: number;
}
...
async function pushFileDiff(bot: Bot, t: LiveTurn, path: string): Promise<void> {
  // Um diff por estado novo do arquivo: fingerprint evita reenviar o
  // acumulado quando o mesmo arquivo é editado várias vezes.
  try {
    const fd = ((t.state as Record<string, unknown>).filediffs as Record<string, { added?: unknown[]; removed?: unknown[] }> | undefined)?.[path];
    const fp = `${fd?.added?.length ?? 0}:${fd?.removed?.length ?? 0}`;
    if (t.diffSent.get(path) === fp) return;
    t.diffSent.set(path, fp);
    const html = await turnRenderDiff(t.state, path, OPENCODE_DIR);
    if (html) await bot.api.sendMessage(t.chatId, html, { parse_mode: "HTML" });
  } catch {
    /* diff é melhor-esforço */
  }
}
...
export async function finishTurn(bot: Bot, t: LiveTurn): Promise<void> {
  ...
  let raw = String((t.state as Record<string, unknown>).out_text ?? "").trim();
  if (!raw) {
    // Backfill (port do núcleo Python): o texto pode existir na sessão mesmo
    // sem deltas capturados (turno anexado, SSE com gap). Só então desiste.
    try {
      const { lastAssistantText } = await import("./opencode");
      raw = sid ? await lastAssistantText(sid) : "";
    } catch (e) {
  ...
  if (!raw) {
    const cards = ((t.state as Record<string, unknown>).tool_cards as Record<string, unknown> | undefined) ?? {};
    console.warn(
...
export async function routeEvent(bot: Bot, ev: ServerEvent): Promise<void> {
  ...
  t.events += 1;
  t.state = res.turn;
  const detail = (res.detail as { tool_phase?: string; tool_id?: string; diff_path?: string } | null) ?? null;
  if (detail?.tool_phase === "ended" && detail.diff_path) await pushFileDiff(bot, t, detail.diff_path);
...
export async function killAllTurns(bot: Bot, chatId?: number): Promise<void> {
  ...
  for (const cid of targets) {
    ...
    const sid = String((t.state as Record<string, unknown>).sid ?? "");
    if (sid) {
      const { interruptSession } = await import("./opencode");
...
export async function replyPermission(bot: Bot, chatId: number, permId: string, reply: "once" | "always" | "reject"): Promise<void> {
  ...
  if (!t || !sid) {
    await bot.api.sendMessage(chatId, "[ERR] sem turno ativo.");
    return;
  }
...
export async function submitQuestions(bot: Bot, chatId: number, rid: string): Promise<void> {
  ...
  await bot.api.sendMessage(chatId, ok ? "[OK] *Resposta enviada ao opencode.*" : "[ERR] *Falha ao enviar resposta.*", {
    parse_mode: "Markdown",
  });
  await pushStatus(bot, t, true);
}
...
export async function askCustom(bot: Bot, chatId: number, rid: string, qidx: number): Promise<void> {
  ...
  await bot.api.sendMessage(chatId, "[EDIT] Digite sua resposta:");
}
