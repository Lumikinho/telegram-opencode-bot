 ...
 * processo com teto de rodadas, relançamento via run.sh.
 */
import { type Bot } from "grammy";
import { writeFileSync } from "node:fs";
import { endpoint, ensureServer, refreshEndpointFromEnv, stopServer } from "./opencode";
import { killAllTurns } from "./turns";

export type RestartTarget = "bot" | "server" | "both";

/** Ganchos opcionais do gateway (ex.: refrescar a lista de sessões e
 * derrubar sids mortos depois de reciclar o servidor). */
export interface RestartHooks {
  didCycleServer?: () => Promise<void>;
}
...
/** Escapa texto dinâmico para HTML do Telegram (só & < >; " crua em
 * texto é válida e fica legível). */
export function escHtml(s: unknown): string {
  return String(s ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

/** 1 mensagem fixa por operação: envia o [...] e devolve o id para editar o
 * resultado no mesmo balão. Fallback para texto puro se o HTML for rejeitado. */
async function sendOpMessage(bot: Bot, chatId: number, htmlText: string): Promise<number | null> {
  try {
  ...
  try {
    const msg = await bot.api.sendMessage(chatId, htmlText.replace(/<[^>]*>/g, ""));
    return msg.message_id;
  } catch (e) {
    console.warn("restart: falha ao enviar mensagem da operação:", e);
    return null;
  }
}

async function editOpMessage(bot: Bot, chatId: number, messageId: number, htmlText: string): Promise<void> {
  try {
    await bot.api.editMessageText(chatId, messageId, htmlText, { parse_mode: "HTML" });
  } catch (e) {
    if (/not modified/i.test(String(e))) return;
    try {
      await bot.api.editMessageText(chatId, messageId, htmlText.replace(/<[^>]*>/g, ""));
    } catch {
      /* melhor esforço */
    }
  }
...
function respawnBotProcess(): void {
  ...
  // Protocolo do supervisor: marker recente = "não relance, o /restart já
  // subiu o substituto". Sem isso um supervisor acorda junto e sobe uma
  // SEGUNDA cópia (conflito de polling).
  try {
    writeFileSync(`${root}/.restart_marker`, String(Math.floor(Date.now() / 1000)));
  } catch {
    /* sem marker o restart ainda funciona; só perde a proteção */
  }
  // Relança o gateway direto (equivale a `make dev-bot`), desanexado.
...
async function restartServerOnly(bot: Bot, chatId: number, hooks?: RestartHooks): Promise<void> {
  const opId = await sendOpMessage(bot, chatId, "[...] <b>Reiniciando o servidor...</b>");
  const done = (htmlText: string) =>
    opId !== null ? editOpMessage(bot, chatId, opId, htmlText) : bot.api.sendMessage(chatId, htmlText, { parse_mode: "HTML" }).catch(() => {});
  try {
    await killAllTurns(bot);
    await cycleServer();
    await hooks?.didCycleServer?.();
    await done(`[OK] <b>Servidor reiniciado</b> — <code>${escHtml(endpoint.url)}</code> OK. O bot continuou no ar.`);
  } catch (e) {
    console.error("restart do servidor falhou:", e);
    await done(`[ERR] <b>Falha no restart do servidor:</b> <code>${escHtml(String(e)).slice(0, 500)}</code>\nVerifique o log: <code>.opencode_bot_server.log</code>`);
  }
}

async function restartBotOnly(bot: Bot, chatId: number): Promise<void> {
  await bot.api.sendMessage(chatId, "[REPEAT] <b>Reiniciando o bot...</b> volto em segundos. O servidor continua no ar.", {
    parse_mode: "HTML",
  }).catch(() => {});
  await killAllTurns(bot);
...
async function restartBoth(bot: Bot, chatId: number, hooks?: RestartHooks): Promise<void> {
  await bot.api.sendMessage(chatId, "[REPEAT] <b>Reiniciando bot + servidor...</b> volto em segundos.", {
    parse_mode: "HTML",
  }).catch(() => {});
  await killAllTurns(bot);
  try {
...
export async function performRestart(bot: Bot, chatId: number, target: RestartTarget, hooks?: RestartHooks): Promise<void> {
  if (target === "bot") await restartBotOnly(bot, chatId);
  else if (target === "server") await restartServerOnly(bot, chatId, hooks);
  else await restartBoth(bot, chatId, hooks);
}

export function restartConfirmKeyboard(): { text: string; data: string }[][] {
  return [
    [
      { text: "[BOT] Só o bot", data: "__restart:bot" },
      { text: "[SRV] Só o servidor", data: "__restart:server" },
    ],
    [{ text: "[REPEAT] Bot + servidor", data: "__restart:both" }],
    [{ text: "[ERR] Cancelar", data: "/status" }],
  ];
}
